	...
	for (int i = 0; i < argc; i++) {
		if (!just_timing && strncasecmp(argv[i], "algorithm=", 10) == 0) {
			...
			*keysize = atol(argv[i] + 5);
			...
			knot_time_t stamp;
			int ret = knot_time_parse("'now'+-#u|'t'+-#u|+-#u|#", strchr(argv[i], '=') + 1, &stamp);
			if (ret < 0) {
...
static void print_timer(const char *name, knot_time_t t, knot_time_print_t format, char separator)
{
	static char buff[100];
	if (knot_time_print(buff, sizeof(buff), t, format) < 0) {
