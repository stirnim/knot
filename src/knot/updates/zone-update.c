...
const knot_rdataset_t *zone_update_to(zone_update_t *update)
{
	...
	printf("zone_update_to %d %p %u\n", update->flags, update->change.soa_to, (update->change.soa_to == NULL ? 0 : knot_soa_serial(&update->change.soa_to->rrs)));
...
int zone_update_increment_soa(zone_update_t *update, conf_t *conf)
{
	...
	int ret = set_new_soa(update, conf_opt(&val));
	return ret;
}
