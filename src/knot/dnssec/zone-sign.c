...
// the reason for having this separate is not updating SOA if everything else is unchanged
// so, the procedure is
// [refresh_DNSKEY_records]->[recreate_nsec]->[sign_zone]->[check_unchanged]->[update_soa]->[sign_soa]
int knot_zone_sign_soa(zone_update_t *update, const zone_keyset_t *zone_keys, const kdnssec_ctx_t *dnssec_ctx)
{
	...
	printf("signing SOA %u\n", knot_soa_serial(&soa_to.rrs));
